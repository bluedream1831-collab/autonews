...
import { Send, TrendingUp, Loader2, Tag, Flame, RefreshCw, Palette, Sparkles, BrainCircuit, Zap, Timer } from 'lucide-react';
...
const InputForm: React.FC<InputFormProps> = ({ onGenerate, isLoading, currentModel, onModelChange }) => {
  ...
  return (
    <div className="space-y-4">
      {/* 快速模型切換器 */}
      <div className="bg-slate-900/50 p-1.5 rounded-2xl border border-slate-800 flex gap-1.5">
        {modelOptions.map((m) => {
          const Icon = m.icon;
          ...
          return (
            <button
              ...
              className={`flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl transition-all duration-300 border ${
                isActive 
                ? `${m.bg} ${m.border} ${m.color} shadow-[0_0_15px_rgba(0,0,0,0.2)]` 
            ...
            >
              ...
              {isActive && <div className={`w-1 h-1 rounded-full ${m.color.replace('text', 'bg')} animate-ping`} />}
            </button>
          );
        })}
      </div>
      ...
      <form onSubmit={handleSubmit} className="bg-slate-850 p-6 rounded-xl border border-slate-700 shadow-xl" autoComplete="off">
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="hidden sm:flex px-3 py-1 bg-slate-900 border border-slate-800 rounded-full items-center gap-1.5">
             <span className="text-[10px] text-slate-500 font-mono uppercase">Engine Status: Online</span>
          </div>
        </div>
